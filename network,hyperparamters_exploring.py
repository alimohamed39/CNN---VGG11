...
from arch_explore import MyCNN
...
class model_train():
  ...
  @staticmethod
  def train_and_validate(model_class, train_dataset, valid_dataset, learning_rates, optimizers, batch_sizes, epochs, device, show_progress=True,patience=5):
                   ...
                   for lr in learning_rates:
                       for optimizer_cls in optimizers:
                           for batch_size in batch_sizes:
                               print(f"Training with Learning Rate = {lr}, Optimizer = {optimizer_cls}, Batch Size = {batch_size} on device {device}")
                               ...
                               for epoch in epochs_range:
                                            ...
                                            if average_val_loss < best_loss:
                                                   ...
                                                   best_hyperparams = (lr, optimizer_cls, batch_size)
                                                   torch.save(model.state_dict(),"model.pth")
                                                   #print(f"Model saved with loss:{best_loss:.4f} with the hyperparameters {best_hyperparams}")


                                            #print(f"Best hyperparameters: Learning Rate = {lr}, Optimizer = {optimizer}, Batch Size = {batch_size}")
                                        ...
                                        #    if epochs_without_improving >= patience :
                                         #          print("Early stopping happened")
                                          #         break
                               print(f"Model saved with loss:{best_loss:.4f} with the lr= {lr},optimizer={optimizer_cls},batch size = {batch_size}")
  ...
  def test(model_class, test_dataset, batch_size,model_path):
       ...
       # model = model_class().to(device)
        ...
        predictions = []
        true_labels = []
        print(f"Testing the data with the batch size:{batch_size}")
        with torch.no_grad():
...
epochs = 15
best_model_path = r"C:\Users\alime\OneDrive - Johannes Kepler Universität Linz\JKU\Semester 2\Python 2\Unit 7 (Project)\files\model(new simpler model to test).pth"
#x = test(model, test_dataset, batch_size=64, model_path=best_model_path)
device = 'cuda' if torch.cuda.is_available() else 'cpu'
best_hyper = model_train.train_and_validate(model, train_dataset, valid_dataset, learning_rates,optimizers,batch_sizes, epochs,device,patience=7)
model_train.test(model, test_dataset,batch_size=best_hyper[2], model_path="model.pth")
