...
class ImagesDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None :
            resized_image = Image.fromarray(resized_image.astype(np.uint8).squeeze(), 'L')

            resized_image = self.transform(resized_image)
        else:
            resized_image = torch.tensor(resized_image, dtype=torch.float32) / 255.0
       # resized_image = transforms.ToTensor()(resized_image)  # Ensure the output is a tensor
...
for file in os.listdir(directory):
    n_f += 1
print(n_f)
dataset = ImagesDataset(directory, 100, 100, int)
print(len(dataset))
